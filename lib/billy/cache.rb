...
module Billy
  class Cache
    ...
    def key(method, orig_url, body, log_key = false)
      url_without_params = format_url(orig_url, true)
      if Billy.config.use_ignore_params
        ignore_params = Billy.config.ignore_params.any? { |ignore_url| ignore_url === url_without_params }
      else
        ignore_params = Billy.config.allow_params.none? { |ignore_url| ignore_url === url_without_params }
      end
